...
def check_directory(url,file,ua):
    ...
    file_check = cmseek.getsource(f'{url}/{file}', ua)
    if file_check[0] == '1' and (
        'Index of' in file_check[1] or 'Last modified</a>' in file_check[1]
    ):
        cmseek.success(
            f'Directory listing enabled in: {cmseek.bold}{cmseek.fgreen}{file}{cmseek.cln}'
        )
        joom_dir_found += 1
