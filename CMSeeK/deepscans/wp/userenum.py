...
def wpauthorenum(ua, url, param):
    ...
    authorsrc = cmseek.getsource(f'{url}/?author={i}', ua)
    if authorsrc[0] == '1':
        if '/author/' in authorsrc[3]: ## Detection using the url redirection
            author = re.findall(r'/author/(.*?)/', str(authorsrc[3]))
            if author != []:
                cmseek.success(
                    f'Found user from redirection: {cmseek.fgreen}{cmseek.bold}{author[0]}{cmseek.cln}'
                )
                wpparamuser.append(author[0])
        elif '/author/' in authorsrc[1]:
            author = re.findall(r'/author/(.*?)/', str(authorsrc[1]))
            if author != []:
                cmseek.success(
                    f'Found user from source code: {cmseek.fgreen}{cmseek.bold}{author[0]}{cmseek.cln}'
                )
                wpparamuser.append(author[0])
...
def start(id, url, ua, ga, source):
    ...
    wpjsonuser = []
    wpjsonsrc = cmseek.getsource(f'{url}/wp-json/wp/v2/users', ua)
    if wpjsonsrc[0] != "1" or 'slug' not in wpjsonsrc[1]:
        ...
        try:
            for user in json.loads(wpjsonsrc[1]):
                ...
                cmseek.success(
                    f"Found user from wp-json : {cmseek.fgreen}{cmseek.bold}"
                    + user['slug']
                    + cmseek.cln
                )
        except:
    ...
    jpapisrc = cmseek.getsource(
        f'https://public-api.wordpress.com/rest/v1.1/sites/{strippedurl}/posts?number=100&pretty=true&fields=author',
        ua,
    )
    if jpapisrc[0] != '1' or 'login' not in jpapisrc[1]:
        ...
        for user in json.loads(jpapisrc[1])['posts']:
            if user['author']['login'] not in str(jpapiuser):
                ...
                cmseek.success(
                    f"Found user from Jetpack api : {cmseek.fgreen}{cmseek.bold}"
                    + user['author']['login']
                    + cmseek.cln
                )
        jpapiuser = list({usr.strip() for usr in jpapiuser})
